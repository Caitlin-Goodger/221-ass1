...
public class Board {
	...
	public boolean isFinished() {
		int last = 0;
		int numCells = dimension * dimension;//Checks number of cells
		for(Cell c : cells) {
			if(c != null) {
				int [] image = c.getImage();
				for(int i =0;i<image.length;i++) {
					int value = image[i];
					if(value-1 !=last) { //Check that the value is one higher than the last one
						return false;
					} 
					last = value;
					if(last == numCells -1) {
			...
			} else { //If cell is null check that it is at the beginning or the end
				if(last != 0 || last != numCells -1) {
					return false;
				}
			}
		}
	...
	public void swap(Location l1, Location l2) {
		//Changed multiplication to dimension
